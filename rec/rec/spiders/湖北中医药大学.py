...
class HbzyySpider(scrapy.Spider):
    name = 'hbzyy_university'
    ...
    def parse(self, response):  # 获取宣讲会url
        url = 'https://hbtcm.91wllm.com/teachin/index?page={}'
    ...
    def prase_detail(self, response):
        ...
        items['Seminar_college'] = '湖北中医药大学'  # 宣讲会举办学校
        ...
        if items['Seminar_time']:
            pass
        else:
            items['Seminar_time'] = response.xpath('//ul[@class="xInfo xInfo-2 cl tInfo-2"]/li[contains(text(),'
                                                   '"举办时间")]/span/text()').extract_first()
        yield items
